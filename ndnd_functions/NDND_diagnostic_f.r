...
ndnddiagnose<-function(Output){
  ...
  # Check growth
  s_b<-Output$Biomass %>% split(as.factor(Output$Biomass[,ncol(Output$Biomass)]))
  sub_bio<-map(s_b,function(x){
    sub<-as.matrix(x)
    sub<-sub[1:(Output$Data$Tmax-1),]
  })
  sub_bio<-do.call(rbind.data.frame,sub_bio)
  Output$Biomass$Years<-rep(1:(Output$Data$Tmax),times=s_length)
  
  growth<-NULL
  for (s in 1:s_length){
    subbio<-Output$Biomass[Output$Biomass$Simulation %in% s,-c(9,10)]
    for (i in 1:(Output$Data$Tmax-1)) {
      a<-subbio[i+1,]/subbio[i,]
      growth<-rbind(growth,a)
    }
  }
  growth$Simulation<-rep(1:s_length,byrow=F,each=(Output$Data$Tmax-1));growth$Years<-rep(1:(Output$Data$Tmax-1),times=s_length)

  gg_growth<-pivot_longer(growth,-c(9,10),names_to = "Species", values_to = "Growth")
  gg_growth$upinertiaconstraint<-Output$Data$Rho %>% exp() %>% rep(times=(Output$Data$Tmax-1)*2)
  gg_growth$lowinertiaconstraint<-Output$Data$Rho %>% rep(times=(Output$Data$Tmax-1)*2) ; gg_growth$lowinertiaconstraint<-exp(-gg_growth$lowinertiaconstraint)
  ...
  for (s in 1:length(Output$Data$Species)) {
    sub_s<-as.matrix(Output$Flows[,grep(pattern=paste("* --> ",Output$Data$Species[s],sep=""),x=colnames(Output$Flows))])
    sum_sp<-as.numeric(apply(sub_s, 1, sum))
    gains<-cbind(gains,sum_sp)
  }
  gains<-cbind(gains,"Simulation"=rep(1:s_length,byrow=F,each=(Output$Data$Tmax-1)),"Years"=rep(1:(Output$Data$Tmax-1),times=s_length))
  ...
  gg.gains$Satiation<-Output$Data$Sgma[-1] %>% rep(times=2*(Output$Data$Tmax-1))
  ...
  Output$Flows$Years<-rep(1:(Output$Data$Tmax-1),times=s_length)
  gg.flows<-pivot_longer(Output$Flows,-c(19,20),names_to = "Flows",values_to = "Values")
  ...
  gg.bio<-pivot_longer(Output$Biomass,-c(9,10),names_to = "Species", values_to = "Biomass")
  ...
  gg.bio$refbio<-rep(Output$Data$Bta,times=Output$Data$Tmax*2)
